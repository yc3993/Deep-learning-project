    ...
    for i in range(batch):
        for n in range(num_of_filters):
            for j in range(channels):
                original_x = x[i, :, :, j]
                if pad:
                    padded_x = np.zeros((height + 2 * pad, width + 2 * pad))
                    padded_x[pad: -1 * pad, pad: -1 * pad] = original_x
                else:
                    padded_x = original_x

                for r in range(0, padded_x.shape[0], stride):
                    if r > padded_x.shape[0] - filter_width:
                        break
                    for c in range(0, padded_x.shape[1], stride):
                        if c > padded_x.shape[1] - filter_height:
                            break

                        out[i, r//stride, c//stride, n] += (w[:, :, j, n] * padded_x[r: r + filter_height, c: c + filter_width]).sum()
            out[i,:,:,n] += b[n]
    return out
...
def avg_pool_forward(x, pool_size, stride):
    ...
    new_width = (height - pool_size) // stride + 1
    ...
    for i in range(batch):
        for j in range(channels):
            ...
            for r in range(0, height, stride):
                if r > height - pool_size:
                    break
                for c in range(0, width, stride):
                    if c > width - pool_size:
                        break

                    out[i, r//stride, c//stride, j] = np.mean(original_x[r: r + pool_size, c: c + pool_size])
